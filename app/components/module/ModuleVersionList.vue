<template>
  <div v-if="versions.length > 0" class="flex flex-wrap gap-1">
    <UBadge v-for="hit in versions" :key="hit.version" color="info" variant="subtle" size="sm" class="rounded-full">
      {{ hit.version }}
    </UBadge>
  </div>
</template>
...
const versions = computed(() => {
  return props.moduleGrouped.hits?.reduce((acc: { version: string }[], hit) => {
